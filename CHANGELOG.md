...
- **Adapted to DeepSeek Harness `dsh-v0.1.7-alpha.1`.** The host deleted the whole `@deepseek-ai/dsh-settings-file` package and inverted the `ctx.settings` contract: there is no plugin namespace-registration surface any more (`installSection` / `register` / `SettingsProvider` / `settings/updated` / `SettingsScope` are all gone), and `@deepseek-ai/dsh-settings` now ships `SettingsForms`, which projects each Loader entry's `runtime.Config` into the Settings form and persists edits on the profile patch. The plugin's configuration surface was migrated accordingly:
  - The exported `Config` schema is now the single source of truth and is shared with the Settings form (`lib/settings-schema.mjs`), replacing the previous "two isomorphic schemas kept in sync" arrangement. Every user-configurable field is marked `volatile()`, which is what makes it editable without remounting the plugin; the host commits edits into the `Volatile` references in place.
  - Reads go through `getLive()` / `resolveConfig()`, which unwrap `Volatile` references (detected with cosmokit's shared `Symbol.for('cosmokit.volatile.write')` protocol) and re-validate only when a reference's snapshot identity actually changed. On hosts without volatile support the same code path reads the plain snapshot as before.
  - `lib/settings-schema.mjs` now also marks `volatile()` with `.extra('volatile', true)` when the resolved schemastery predates the `.volatile()` method (3.18.2), so the module still imports on the older lines instead of throwing at import time.
  - `/rewind config <id>` writes back through `SettingsForms.replace(entryId, section)` on the new host (`persisted in the profile patch`) and keeps the namespace-scope write-back on hosts that still expose `settings.register`. When neither surface exists the existing in-process fallback still reports itself as non-durable rather than pretending to succeed.
  - Config bounds moved into the schema (`min`/`max`/`step(1)`), so the host rejects out-of-range and non-integer values **before** persisting them; `validateCheckpointSettings` stays as the pure cross-field validator called from `resolveConfig`. The host cookbook's `schema.check()` does not exist in the schemastery version this host ships (3.18.3), so no `.check()` was introduced.
- **Typert strict codecs collapsed to a single face.** `TypertCodec` no longer has a `schema` field — the runtime validator only accepts `create()` and throws `… has no create() factory` without it. The seven descriptors in `client/index.js` and `lib/wire.mjs`'s `strictCodec()` now carry only `create` (the `schema` property was dead code on the new host, not a second contract).
- **Message sources use a producer-owned `kind`.** The host removed the `kind: 'plugin'` catch-all from `MessageSourceMap` and the session-format v3→v4 row admission rejects `kind === 'plugin'` outright. The rewind notice injected into a replayed child session now declares `kind: 'dsh-checkpoint-rewind'` through a `MessageSourceMap` declaration merge in `types.d.ts`, with `{ form: 'notice', summary: 'rewind' }` in place of the undeclared `rewind-notice` form (the shape the host's own `tool-jobs` plugin uses).
- Dev/test dependencies now pin the `@deepseek-ai/dsh-*` alpha line (`0.1.7-alpha.1`) instead of `0.1.5-rc.2`, together with `@deepseek-ai/cordis@^4.0.3` (the first cordis that exports `Volatile`), `@deepseek-ai/cordis-plugin-include@^1.0.8` and `@deepseek-ai/cordis-plugin-loader@^1.0.4`. The tree resolves `@deepseek-ai/schemastery@3.18.3` (the first version with `volatile()`) and `@deepseek-ai/cosmokit@1.8.4` (the first with `Volatile`/`isVolatile`). `@deepseek-ai/dsh-code-runtime` was dropped from `devDependencies`: it existed only to satisfy a peer of `dsh-tools@0.1.5-rc.2`, and on the alpha line that peer moved to `@deepseek-ai/dsh-ptc-runtime`; the newly required peers `dsh-ptc-runtime`, `dsh-sandbox`, `dsh-sandbox-policy` and `dsh-workspace` were added instead. No peer range for consumers was tightened: `>=0.1.2-rc.1 <0.2.0 || >=0.1.5-alpha.1 <0.2.0 || >=0.1.6-0 <0.2.0` already admits every line named above.
- `dshWorkshop.compatibility.dshVersions` records `0.1.7-alpha.1`, and the monthly Compat workflow matrix now smoke-tests `0.1.5-rc.2`, `0.1.6-alpha.2` and `0.1.7-alpha.1` (both settings generations stay covered).

### Fixed

- The real-Loader composition probe (`scripts/loader-runner.mjs`) again fails for the *right* reason on a rejected config. `dsh-v0.1.7-alpha.1`'s loader awaits its entry tasks with `Promise.allSettled` and therefore no longer rethrows a failed row (`Fiber.await()` still throws, `Tree.await()` no longer does), so an invalid plugin config left the composition "successfully" running with the row silently unmounted. The runner now finds the failed entry's fiber and rethrows its recorded error, keeping the invalid-config and default-export regression cases meaningful. The plugin's own behavior is unchanged: an illegal config still fails loudly while its fiber is being resolved.
