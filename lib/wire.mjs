...
/** 只读调用参数 codec 的共享 zod schema（from/to/id 三个参数同构）。 */
const checkpointIdRefSchema = z.string().min(1)
/** timeline limit 参数 schema（网关 exact-args 检查只认显式 acceptsUndefined）。 */
const timelineLimitSchema = z.number().int().min(1).max(200).optional()

/**
 * 0.1.7-alpha.1 typert 契约：strict codec **只有** create() 工厂一个面
 * （`schema` 字段已从 TypertCodec 删除；运行时校验器只认 create，缺它即抛
 * "… has no create() factory"）。网关以 `codec.create().parse(value)` 消费，
 * 因此工厂返回 zod schema 自身即可（zod 实例自带 .parse）。
 * @param {string} typeSymbol - typert 类型符号（如 `${PACKAGE}/types#TimelineLimit`）。
 * @param {import('zod').ZodType} schema - 该 codec 的权威 zod 校验器。
 * @returns {{ mode: 'strict', typeSymbol: string, create: () => import('zod').ZodType }}
 */
...
export const TIMELINE_DESCRIPTOR = Object.freeze({
  ...
  parameters: Object.freeze([Object.freeze({
    ...
    codec: Object.freeze(strictCodec(`${PACKAGE}/types#TimelineLimit`, timelineLimitSchema)),
  })]),
  result: Object.freeze(strictCodec(`${PACKAGE}/types#TimelineSnapshot`, timelineSnapshotSchema)),
...
export const DIFF_DESCRIPTOR = Object.freeze({
  ...
  parameters: Object.freeze([
    Object.freeze({
      ...
      wire: 'from',
      source: 'json',
      codec: Object.freeze(strictCodec(`${PACKAGE}/types#CheckpointIdRef`, checkpointIdRefSchema)),
    }),
    Object.freeze({
      name: 'to',
      wire: 'to',
  ...
  result: Object.freeze(strictCodec(`${PACKAGE}/types#DiffResult`, diffResultSchema)),
  sourceLocation: SOURCE_LOCATION,
})
...
export const RESTORE_PREVIEW_DESCRIPTOR = Object.freeze({
  ...
  result: Object.freeze(strictCodec(`${PACKAGE}/types#RestorePreviewResult`, restorePreviewResultSchema)),
  sourceLocation: SOURCE_LOCATION,
})
