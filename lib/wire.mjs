...
/**
 * alpha.2 typert 契约：strict codec 必须携带 create() 工厂（loader 校验存在，
 * 网关以 `codec.create().parse(value)` 消费）。返回 zod schema 自身即可
 * （zod 实例自带 .parse）；rc.2 不检查该字段，多余属性无害 → 双档兼容。
 * @param {string} typeSymbol - typert 类型符号（如 `${PACKAGE}/types#TimelineLimit`）。
 * @param {import('zod').ZodType} schema - 该 codec 的权威 zod 校验器。
 * @returns {{ mode: 'strict', typeSymbol: string, schema: import('zod').ZodType, create: () => import('zod').ZodType }}
 */
