 ...
 *   | {kind: 'invalid', message: string}} 解析结果。
 */
export function parseRewindInput(rawInput) {
  ...
  if (/^diff\b/iu.test(input)) {
    return { kind: 'invalid', message: 'usage: /rewind diff <checkpoint-a> <checkpoint-b>' }
  }
  const targeted = /^(workspace|session|config|all)\s+(.+)$/iu.exec(input)
  if (targeted !== null) {
    return { kind: 'target', target: targeted[1].toLowerCase(), input: targeted[2].trim() }
  }
  if (/^(workspace|session|config|all)\b/iu.test(input)) {
    return { kind: 'invalid', message: 'usage: /rewind [workspace|session|config|all] <id-prefix | step <N> | latest>' }
  }
