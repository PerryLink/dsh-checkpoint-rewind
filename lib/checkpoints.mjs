 ...
 * 类型别名指向 types.d.ts 的对外契约（单一事实源）。
 * @typedef {import('../types.d.ts').CheckpointRecord} CheckpointRecord
 */
...
export function nearestCheckpointAtOrBefore(records, boundarySeq) {
  ...
  for (const record of records) {
    ...
    if (best === undefined || (best.stepEndSeq ?? -1) < seq) best = record
  }
  return best
}
 ...
 * @param {{maxSnapshots: number, maxSnapshotBytes: number, keepNewestPerSession?: boolean, liveSessionIds?: Set<string>|string[], isMissing?: (record: CheckpointRecord) => boolean, exists?: (record: CheckpointRecord) => boolean}} opts - 配额。
 * @returns {{ids: string[], byRule: {missing: string[], maxSnapshots: string[], maxSnapshotBytes: string[]}}} 待删 id（最旧优先）与触发规则拆分。
 */
export function prunePlan(entries, opts) {
  ...
  const liveSessionIds = opts.liveSessionIds instanceof Set
    ...
    : Array.isArray(opts.liveSessionIds)
      ...
      : undefined
  let isMissing
  if (typeof opts.isMissing === 'function') {
    isMissing = opts.isMissing
  } else if (typeof opts.exists === 'function') {
    const exists = opts.exists
    isMissing = /** @param {CheckpointRecord} record */ (record) => !exists(record)
  }
  ...
  for (const list of bySession.values()) {
    ...
    if (newest !== undefined) newestBySession.set(newest.sessionId, newest.id)
  }
...
/**
 * /rewind 输入解析结果形态（恢复目标形态可带 files?: string[]）。
 * @typedef {{kind: 'list'} | {kind: 'latest', files?: string[]} | {kind: 'step', step: number, files?: string[]} | {kind: 'clear', all?: boolean}
 *   | {kind: 'preview', target: string} | {kind: 'diff', a: string, b: string}
 *   | {kind: 'id', input: string, files?: string[]} | {kind: 'target', target: 'workspace'|'session'|'config'|'all', input: string, files?: string[]}
 *   | {kind: 'invalid', message: string}} RewindParseResult
 */
 ...
 * @param {string} rawInput - 命令原始输入（未 trim）。
 * @returns {RewindParseResult} 解析结果。
 */
export function parseRewindInput(rawInput) {
  ...
  if (input === '') result = { kind: 'list' }
    ...
    if (lower === 'list') result = { kind: 'list' }
      ...
      const stepMatch = /^step\s+(\d+)$/iu.exec(input)
      if (stepMatch !== null) {
        ...
        if (diff !== null) {
          ...
          if (targeted !== null) {
            result = { kind: 'target', target: /** @type {'workspace'|'session'|'config'|'all'} */ (targeted[1].toLowerCase()), input: targeted[2].trim() }
 ...
 * @param {readonly {type: string, data: unknown, seq: number}[]} events - 会话事件。
...
export function stepEndSeqOf(events, step) {
  ...
  for (const event of events) {
    if (event.type === 'step/end' && /** @type {{step?: number}} */ (event.data).step === step) seq = event.seq
  }
  return seq
}
 ...
 * @param {{timeFormatter?: (ms: number) => string, now?: number, total?: number, command?: string}} [opts] - 渲染选项（测试注入；command 决定提示行文案）。
 * @returns {string} 多行文本。
 */
 ...
 * @param {{limit?: number}} [opts] - {limit}（默认 20，测试注入）。
 * @returns {string} 多行文本。
 */
 ...
 * @param {{files?: {changed: number, added: number, removed: number, names?: string[], truncated?: boolean}, configDiff?: {changed: boolean, lines: number, text: string}, session?: {fromSeq: number, toSeq: number, fromTurn?: number, fromStep?: number, toTurn?: number, toStep?: number, dropped: number}, note?: string}} diff - 对比结果。
 * @param {{limit?: number}} [opts] - {limit}（文件清单上限，默认 20）。
 * @returns {string} 多行文本。
 */
 ...
 * @param {{verifyObjectExists?: (workspace: object, ref: string) => Promise<boolean>}} provider - 快照 provider 实例（支持 verifyObjectExists 方法）。
