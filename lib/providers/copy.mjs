...
import { createGitignoreReader } from '../ignore-files.mjs'
import { COPY_MANIFEST, COPY_TMP_SUFFIX, PROVIDERS } from '../constants.mjs'
import { providerFailed, snapshotAborted, snapshotBudgetExceeded } from '../errors.mjs'
...
export class CopySnapshotProvider {
  ...
  #verifyByHash
  /** @type {() => boolean} 是否读取工作区 .gitignore（默认 true）。 */
  #respectGitignore
  /** @type {() => number} 单次快照遍历的文件数预算（超限中止）。 */
  #maxSnapshotFiles
  /** @type {() => number} 单次快照遍历的墙钟预算（毫秒，超限中止）。 */
  #snapshotTimeoutMs
   ...
   *   respectGitignore, maxSnapshotFiles, snapshotTimeoutMs}；
   *   每项可为值或返回值的函数（惰性求值以跟随 settings 热更新）。
   */
  constructor(deps) {
    ...
    this.#respectGitignore = typeof deps.respectGitignore === 'function'
      ? () => deps.respectGitignore() !== false
      : () => deps.respectGitignore !== false
    this.#maxSnapshotFiles = typeof deps.maxSnapshotFiles === 'function'
      ? () => deps.maxSnapshotFiles()
      : () => deps.maxSnapshotFiles
    this.#snapshotTimeoutMs = typeof deps.snapshotTimeoutMs === 'function'
  ...
  snapshot(workspace, ctx) {
    return withLock(this.#chains, workspace.key, async () => {
      ...
      const notes = []
      const { files, bytes, warnings } = await this.#walk(workspace, tmp, previous, notes, ctx)
      notes.push(...warnings)
   ...
   */
  /**
   * 遍历工作区并把文件物化到 tmp（hardlink 复用优先）。三重中止线：
   * 1. signal（会话回合取消/宿主卸载）→ SNAPSHOT_ABORTED；
   * 2. 预算（文件数/墙钟超限）→ SNAPSHOT_BUDGET_EXCEEDED——快照
   *    是安全网，绝不能比被保护的操作更贵（真实事故：87 万文件全量拷贝）；
   * 3. 单文件不可读/不可拷 → 记警告跳过（快照仍可完成）。
   * .gitignore（根+嵌套）与静态 excludeGlobs 一同在入目录前剪掉整棵子树——
   * 忽略语义先于 stat，巨型被忽略目录的成本就近于零。
   * @param {import('./definition.mjs').WorkspaceInfo} workspace - 目标工作区。
   * @param {string} tmp - 本次快照的 *.tmp 目录。
   * @param {ReturnType<typeof assertManifest>|undefined} previous - 上一快照清单。
   * @param {string[]} notes - 备注收集器。
   * @param {{signal?: AbortSignal}} ctx - 捕获上下文。
   * @returns {Promise<{files: Array<{rel: string, size: number, mtimeMs: number, mode: number, hash?: string}>, bytes: number, warnings: string[]}>}
   */
  async #walk(workspace, tmp, previous, notes, ctx) {
    const files = []
    const warnings = []
    let bytes = 0
    ...
    const gitignore = this.#respectGitignore() ? createGitignoreReader(workspaceRoot) : undefined
    if (gitignore !== undefined) await gitignore.enterDirectory('')
    const maxFiles = this.#maxSnapshotFiles()
    const timeoutMs = this.#snapshotTimeoutMs()
    const startedAt = Date.now()

    const checkBudgets = () => {
      if (ctx.signal?.aborted === true) throw snapshotAborted('capture cancelled')
      if (Number.isSafeInteger(maxFiles) && maxFiles > 0 && files.length > maxFiles) {
        throw snapshotBudgetExceeded('files', { limit: maxFiles, visited: files.length })
      }
      const elapsedMs = Date.now() - startedAt
      if (Number.isSafeInteger(timeoutMs) && timeoutMs > 0 && elapsedMs > timeoutMs) {
        throw snapshotBudgetExceeded('timeout', { limitMs: timeoutMs, elapsedMs })
      }
    }

    const visit = async (dir, relDir = '') => {
      if (gitignore !== undefined) await gitignore.enterDirectory(relDir)
      let entries
      try {
      ...
      for (const entry of entries) {
        ...
        if (gitignore?.isIgnored(relPath) === true) continue
