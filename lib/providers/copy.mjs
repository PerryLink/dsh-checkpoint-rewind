...
/**
 * 清单形状（assertManifest 校验后的权威类型）。
 * @typedef {{rel: string, size: number, mtimeMs: number, mode: number, hash?: string}} ManifestFile
 * @typedef {{id: string, base: string|null, files: ManifestFile[], bytes: number}} SnapshotManifest
 */
 ...
 * @returns {SnapshotManifest} 校验过的清单。
 */
function assertManifest(value) {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) {
    throw new Error('checkpoint manifest is not an object')
  }
  const manifest = /** @type {{id?: unknown, base?: unknown, files?: unknown, bytes?: unknown}} */ (value)
  ...
  for (const entry of manifest.files) {
    ...
    const entryObject = /** @type {Record<string, unknown>} */ (entry)
    for (const key of manifestEntrySchema) {
      const value2 = entryObject[key]
      if (key === 'rel') {
    ...
    if (entryObject.hash !== undefined && (typeof entryObject.hash !== 'string' || entryObject.hash.length === 0)) {
      throw new Error('checkpoint manifest entry hash is invalid')
    }
    if (typeof entryObject.rel !== 'string' || seen.has(entryObject.rel)) throw new Error(`checkpoint manifest duplicates ${String(entryObject.rel)}`)
    seen.add(entryObject.rel)
  }
  return /** @type {SnapshotManifest} */ (manifest)
}
...
export class CopySnapshotProvider {
  ...
  #excluded
  /** @type {() => boolean} 内容哈希校验开关（解析器形态：构造时收敛为函数）。 */
  #verifyByHash
  /** @type {Map<string, Promise<unknown>>} 每工作区串行链。 */
  #chains = new Map()

  /**
   * @param {{snapshotDir: string | (() => string), excludeGlobs: string[] | (() => string[]), verifyByHash?: boolean | (() => boolean)}} deps - snapshotDir 为绝对路径
   *   或返回绝对路径的函数（首次使用时求值，非法/缺失响亮失败）。
   */
  constructor(deps) {
    const snapshotDir = deps.snapshotDir
    this.#snapshotDir = typeof snapshotDir === 'function' ? snapshotDir : () => path.resolve(snapshotDir)
    const excludeGlobs = deps.excludeGlobs
    this.#excluded = typeof excludeGlobs === 'function'
      ? (relPath) => makeExcluder(excludeGlobs() ?? [])(relPath)
      : makeExcluder(/** @type {string[]} */ (excludeGlobs ?? []))
    const verifyByHash = deps.verifyByHash
    this.#verifyByHash = typeof verifyByHash === 'function' ? verifyByHash : () => verifyByHash === true
  }
   ...
   * @param {import('./definition.mjs').WorkspaceInfo} [workspace] - 目标工作区（未使用；保留消费方一致性）。
  ...
  async restore(workspace, ref, signal, files) {
    return withLock(this.#chains, workspace.key, async () => {
      ...
      if (files !== undefined) {
        ...
        targetEntries = files.map(rel => /** @type {ManifestFile} */ (byRel.get(rel)))
      }
  ...
  async diffFiles(workspace, fromRef, toRef) {
    ...
    return withLock(this.#chains, workspace.key, async () => {
      ...
      /** @param {{hash?: string, size: number}} a @param {{hash?: string, size: number}} b */
      ...
      for (const rel of toFiles.keys()) {
        ...
        } else if (!sameContent(before, /** @type {ManifestFile} */ (toFiles.get(rel)))) {
          names.add(rel)
  ...
  /** 快照根 + 工作区子目录。 */
  /** @param {import('./definition.mjs').WorkspaceInfo} workspace */
  #baseDir(workspace) {
...
/** @param {ManifestFile[]} previous @param {ManifestFile[]} current */
...
/**
 * 构造 copy provider。
 * @param {{snapshotDir: string | (() => string), excludeGlobs: string[] | (() => string[]), verifyByHash?: boolean | (() => boolean)}} deps - 构造依赖。
