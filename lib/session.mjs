 ...
 * @param {readonly {type: string, seq: number}[]} events - 会话事件日志。
 ...
 * @param {readonly {type: string, seq: number}[]} events - 会话事件日志（元素含 seq）。
 * @param {number|undefined} boundary - sessionBoundaryAt 的结果。
 * @returns {import('@deepseek-ai/dsh-session').SessionEvent[]} 可交给 sessions.create({seed}) 的事件前缀。
 */
export function replaySeedOf(events, boundary) {
  if (typeof boundary !== 'number') return []
  return /** @type {import('@deepseek-ai/dsh-session').SessionEvent[]} */ (events.slice(0, boundary + 1))
}
...
export function turnAt(events, cursor) {
  ...
  for (const event of events) {
    if (event.seq > cursor) break
    if (event.type === 'turn/start') turn = /** @type {{ data?: { turn?: number } }} */ (event).data?.turn
  }
  return turn
}
 ...
 * @returns {{fromSeq: number, toSeq: number, fromTurn?: number, fromStep?: number, toTurn?: number, toStep?: number, dropped: number}} 游标差摘要。
 */
export function sessionDelta(from, to) {
  /** @param {{seq: number, turn?: number, step?: number}} record @param {number|undefined} fallback */
