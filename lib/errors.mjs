...
/**
 * 快照遍历超预算（文件数/耗时/实拷贝字节超限）。快照是安全网，绝不能比
 * 被保护的操作更贵：超限即中止本次捕获（不产出检查点），调用链继续。
 * @param {string} kind - 'files' | 'timeout' | 'bytes'。
 * @param {object} details - 结构化事实（limit/visited/elapsedMs/bytes 等）。
 * @returns {RewindError} code=SNAPSHOT_BUDGET_EXCEEDED。
 */
export function snapshotBudgetExceeded(kind, details) {
  return new RewindError(
    ERROR_CODES.SNAPSHOT_BUDGET_EXCEEDED,
    `snapshot budget exceeded (${kind}): ${JSON.stringify(details)}`,
    { kind, ...details },
  )
}

/**
 * 快照遍历被中止（会话回合取消/中断或宿主卸载）。
 * @param {string} reason - 中止原因。
 * @returns {RewindError} code=SNAPSHOT_ABORTED。
 */
export function snapshotAborted(reason) {
  return new RewindError(ERROR_CODES.SNAPSHOT_ABORTED, `snapshot aborted: ${reason}`, { reason })
}
