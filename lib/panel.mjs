...
/**
 * 插件内部依赖（index.mjs apply 组装；getTable 惰性解析存储表）。
 * @typedef {{
 *   getTable: () => Promise<import('@deepseek-ai/dsh-storage-domain').KvTable<string, import('../types.d.ts').CheckpointRecord>>,
 *   ops: Promise<void>,
 *   registry: {get: (name: string) => import('./providers/definition.mjs').SnapshotProvider | undefined},
 *   getLive: () => Required<import('../types.d.ts').CheckpointConfig>,
 * }} PanelDeps
 */

/** 时间线行（wire 形状；领域记录 → 面板只读视图，绝不透出 ref/配置全文）。 */
/** @param {import('../types.d.ts').CheckpointRecord} record */
...
export class CheckpointPanelService extends TypertRemoteService {
   ...
   * @returns {Promise<{rows: Array<{id: string, sessionId: string, time: number, seq: number, kind: string, provider: string, triggerTool: string, turn: number, step: number, files: number, bytes: number, tree: string | null, note: string | null, sessionBoundary: number | null}>, total: number, bytes: number, maxSnapshots: number | undefined, maxSnapshotBytes: number | undefined, diffRenderer: string, selectiveRestore: boolean}>} 时间线快照。
   */
  async timeline(limit) {
    ...
    const capped = typeof limit === 'number' && Number.isInteger(limit) && limit >= 1 ? Math.min(limit, TIMELINE_MAX_ROWS) : LIMITS.MAX_LIST_LIMIT
    ...
    return {
      ...
      diffRenderer: Object.values(DIFF_RENDERER_MODES).includes(/** @type {any} */ (live.diffRenderer)) ? /** @type {string} */ (live.diffRenderer) : DIFF_RENDERER_MODES.PAIRWISE,
   ...
   * @returns {Promise<{from: string, to: string, error: string | null, files: {changed: number, added: number, removed: number, names: string[], truncated: boolean, entries: Array<{path: string, status: string}>}, configDiff: {changed: boolean, lines: number, text: string}, session: {fromSeq: number, toSeq: number, fromTurn: number | null, fromStep: number | null, toTurn: number | null, toStep: number | null, dropped: number}, fromNote: string | null, toNote: string | null}>} 对比结果（wire 形状）。
   */
  async diff(from, to) {
    /** @param {string} error */
    const empty = (error) => ({
    ...
    try {
      ...
      if (fromRecord === undefined || toRecord === undefined) {
        return empty('unknown checkpoint id')
      }
      ...
      const delta = sessionDelta(fromRecord, toRecord)
      return {
        from: fromRecord.id,
        to: toRecord.id,
        error: null,
        ...
        session: {
          ...
          toSeq: delta.toSeq,
          fromTurn: delta.fromTurn ?? null,
          fromStep: delta.fromStep ?? null,
          toTurn: delta.toTurn ?? null,
          toStep: delta.toStep ?? null,
   ...
   * @returns {Promise<{id: string, provider: string | null, error: string | null, files: Array<{path: string, bytes: number}>, totalBytes: number, truncated: boolean}>} 预览结果。
   */
...
/** @param {Array<{time: number, seq: number}>} rows */
