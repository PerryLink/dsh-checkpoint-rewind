 ...
 * @param {{ctx: {get: (name: string) => unknown}, confirmVia: string, summary: string, question?: string, approveLabel?: string, approveDescription?: string, cancelLabel?: string}} deps - 依赖与文案。
 * @param {unknown} agent - 命令所属 agent（userQuestions 路由 + approval 审计归属）。
...
export async function confirmRewind(deps, agent, signal) {
  ...
  const agentRef = /** @type {{session?: {snapshotEvents?: () => Array<{type: string, data?: object}>, events?: Array<{type: string, data?: object}>}} | null | undefined} */ (agent)
  const text = {
  ...
  if (channel === CONFIRM_CHANNELS.USER_QUESTIONS) {
    const service = /** @type {{ask?: (input: object) => Promise<{answers?: Array<{id: string, selected?: unknown, custom?: unknown}>}>}} */ (deps.ctx.get('userQuestions'))
  ...
  if (channel === CONFIRM_CHANNELS.APPROVAL) {
    ...
    if (!hasOpenTurn(typeof agentRef?.session?.snapshotEvents === 'function' ? agentRef.session.snapshotEvents() : agentRef?.session?.events)) {
      return {
        allowed: false,
        channel,
    ...
    const service = /** @type {{request?: (input: object) => Promise<string>}} */ (deps.ctx.get('approval'))
 ...
 * @param {unknown} session - Session（缺失即跳过；宿主 append 签名各异，运行时探测）。
...
export function maybeAppendSessionEvent(session, type, data, gate, warn) {
  const target = /** @type {{append?: (type: string, data: object, envelope?: {ignorable: boolean}) => unknown}|null|undefined} */ (session)
  if (target === null || target === undefined) return undefined
  if (typeof target.append !== 'function') return undefined
  const decision = gate(type)
  if (!decision.append) return undefined
  try {
    return target.append(type, data, decision.ignorable ? { ignorable: true } : undefined)
  } catch (error) {
