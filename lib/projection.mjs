...
//
// rc.6 现实：checkpoint/* 事件经自适应门（lib/gate.mjs）append，当前宿主构建
// 不收录这些类型，因此该单元在 rc.6 上恒为空列表；宿主收录词汇后无需改插件
...
function wireOf(data) {
  return {
    ...
    ...(data.forkSeq === undefined ? {} : { forkSeq: data.forkSeq }),
  }
}
  ...
  switch (event.type) {
    ...
    case SESSION_EVENTS.BOUND: {
      ...
      const patched = {
        ...
        ...(event.data.forkSeq === undefined ? {} : { forkSeq: event.data.forkSeq }),
      }
    ...
    case SESSION_EVENTS.REWIND: {
      ...
      if (current === undefined) return state
      return { ...state, [event.data.checkpointId]: { ...current, rewindOutcome: event.data.outcome } }
    }
    default:
      return state
  }
...
/**
 * 'checkpoints' 投影单元（ProjectionDefinition）。
 */
export const checkpointsProjectionDefinition = {
  key: 'checkpoints',
  schema: checkpointsWireSchemaList,
  stateVersion: 0,
  init,
  apply,
  view,
