...
[![dshfind](https://dshfind.com/api/badge/PerryLink/dsh-checkpoint-rewind?metric=downloads&lang=pt)](https://dshfind.com/pt/plugins/PerryLink/dsh-checkpoint-rewind?ref=badge)
...
| Superfície | Status |
|---|---|
| Harness | DeepSeek Harness `dsh-v0.1.7-alpha.1` (tag do GitHub, verificado em 2026-09-22; pin npm `0.1.7-alpha.1`, peers `>=0.1.2-rc.1 <0.2.0 || >=0.1.5-alpha.1 <0.2.0 || >=0.1.6-0 <0.2.0 || >=0.1.7-0 <0.2.0`) (adaptado em 2026-09-22): o host removeu `@deepseek-ai/dsh-settings-file` e substituiu a superfície de registro de namespaces de `ctx.settings` por `SettingsForms`, que projeta o `Config` de cada entrada do Loader no formulário de Configurações e persiste as edições no patch do perfil — os campos de configuração do plugin agora são declarados `volatile()` e lidos ao vivo, os codecs estritos do typert carregam apenas sua fábrica `create()`, e o aviso de rebobinamento declara seu próprio `kind` de origem de mensagem. O caminho legado `settings.register` é mantido para hosts `0.1.5-rc.2`/`0.1.6-alpha.2`. Verificado em 2026-09-22 contra o checkout do `dsh-v0.1.7-alpha.1` (typecheck contra a superfície de tipos do alpha.1 + suíte de unidades completa + integração headless montada). |
| Node | `^22.19.0 \|\| >=24.0.0` |
...
5. **Reversão de sessão por fork** — a reversão de sessão reproduz eventos até o limite do checkpoint pela primitiva oficial `SessionStore.fork`, criando uma nova sessão filha (com fallback para o caminho `sessions.create` com semente quando o host não tem fork ou o checkpoint não tem limite); a sessão original mantém seu histórico completo.
...
- A reversão de sessão **bifurca (fork) uma nova sessão filha** no limite do checkpoint; ela nunca reescreve nem trunca a sessão original.
