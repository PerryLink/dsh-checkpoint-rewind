...
[![dshfind](https://dshfind.com/api/badge/PerryLink/dsh-checkpoint-rewind?metric=downloads&lang=hi)](https://dshfind.com/hi/plugins/PerryLink/dsh-checkpoint-rewind?ref=badge)
...
| सतह | स्थिति |
|---|---|
| Harness | DeepSeek Harness `dsh-v0.1.7-alpha.1` (GitHub टैग, 2026-09-22 को सत्यापित; npm पिन `0.1.7-alpha.1`, peers `>=0.1.2-rc.1 <0.2.0 || >=0.1.5-alpha.1 <0.2.0 || >=0.1.6-0 <0.2.0 || >=0.1.7-0 <0.2.0`) (2026-09-22 को अनुकूलित): होस्ट ने `@deepseek-ai/dsh-settings-file` हटा दिया और `ctx.settings` की नेमस्पेस-पंजीकरण सतह को `SettingsForms` से बदल दिया, जो हर Loader प्रविष्टि के `Config` को सेटिंग्स फ़ॉर्म में प्रक्षेपित करता है और संपादन profile patch पर सहेजता है — इसलिए प्लगइन कॉन्फ़िग फ़ील्ड अब `volatile()` घोषित हैं और जीवंत पढ़े जाते हैं, typert strict codec केवल अपनी `create()` फ़ैक्ट्री रखते हैं, और रीवाइंड सूचना अपना स्वयं का संदेश-स्रोत `kind` घोषित करती है। `0.1.5-rc.2`/`0.1.6-alpha.2` होस्ट के लिए विरासती `settings.register` पथ बना रहता है। 2026-09-22 को `dsh-v0.1.7-alpha.1` चेकआउट के विरुद्ध सत्यापित (alpha.1 टाइप सतह के विरुद्ध typecheck + पूर्ण यूनिट सूट + असेंबल्ड headless एकीकरण)। |
| Node | `^22.19.0 \|\| >=24.0.0` |
...
5. **fork-आधारित सत्र रोलबैक** — सत्र रोलबैक आधिकारिक `SessionStore.fork` प्रिमिटिव से चेकपॉइंट सीमा तक की घटनाएँ दोहराकर एक नया चाइल्ड सत्र बनाता है (होस्ट में fork न होने या चेकपॉइंट में सीमा न होने पर `sessions.create` सीड पथ पर फ़ॉलबैक); मूल सत्र अपना पूरा इतिहास रखता है।
...
- सत्र रोलबैक चेकपॉइंट सीमा पर एक **नया चाइल्ड सत्र fork करता है**; यह मूल सत्र को कभी दोबारा नहीं लिखता या काटता।
